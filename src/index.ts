...
export default defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      ...
      let labelNodes;
      if (defaultSlot) {
        labelNodes = defaultSlot({ node: props.node, data: props.data });
      } else {
        labelNodes = [
          nodeLabelSlot
            ? nodeLabelSlot({ node: props.node, data: props.data })
            : h('span', { class: 'element-tree-node-label' }, props.node.label),
          props.showLabelLine
            ? h('span', { class: 'element-tree-node-label-line' })
            : null,
          afterNodeLabelSlot
            ? afterNodeLabelSlot({ node: props.node, data: props.data })
            : null,
        ].filter(Boolean);
      }
        ...
        lineNodes.push(
          h('span', {
            class: {
              ...
              'last-node-isLeaf-line': lastnodeArr[i] && props.node.level - 1 === i,
            },
      ...
      return h(
        ...
        [...(Array.isArray(labelNodes) ? labelNodes : [labelNodes]), ...lineNodes, ...(horLineNode ? [horLineNode] : [])].filter(
          (n) => n !== null
        )
      );
    };
